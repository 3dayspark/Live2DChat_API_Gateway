...
class SimpleTextGenerator:
    ...
    async def generate_text_response_for_api(self,
        ...
        emotion_instruction = "此外，请务必在你的回复最开头添加一个情感标签，格式为[emotion:情感名]。可选的情感名只有：happiness, sadness, anger, disgust, like, surprise, fear, idle, thinking。例如：[emotion:happiness]你好啊！"
        ...
        final_llm_prompt = f"{llm_base_role_prompt} {emotion_instruction} {llm_instruction_for_this_turn}"
        ...
        if use_modelscope:
            ...
            try:
                ...
                if not raw_response_text:
                    ...
                    return ("（AI响应未生成内容）", actual_speaker_index, "idle")
            ...
            try:
                ...
                if "candidates" in response_json and response_json["candidates"]:
                    ...
                    if "finishReason" in candidate and candidate["finishReason"] == "MAX_TOKENS":
                        ...
                        return ("（AI响应因达到最大Token限制而未生成内容）", actual_speaker_index, "idle")
        ...
        emotion_tag = "idle"
        pattern = r'[\【\[<]\s*(?:emotion|情感)\s*[:：=]\s*([a-zA-Z]+)\s*[\】\]>]\s*'
        
        matches = re.findall(pattern, raw_response_text, flags=re.IGNORECASE)
        if matches:
            raw_emotion = matches[0].lower()
            # マッチしたすべての感情タグをテキストから完全に除去する
            raw_response_text = re.sub(pattern, '', raw_response_text, flags=re.IGNORECASE).strip()
            # LLMがタグの直後に句読点を生成した場合（例:[emotion:disgust]。そうだね）、先頭の不要な記号を削除する
            ...
            emotion_tag = TARGET_MAP.get(raw_emotion, "idle")
            
            logger.info(f"[情感映射] LLM输出: {raw_emotion} -> 转换给前端: {emotion_tag}")
            
        return (raw_response_text, actual_speaker_index, emotion_tag)